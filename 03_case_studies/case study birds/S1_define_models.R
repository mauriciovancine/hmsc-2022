...
da = read.csv(file.path(dataDir, "data.csv"), stringsAsFactors=TRUE)
...
alltraits = read.csv(file.path(dataDir, "traits.csv"), stringsAsFactors = TRUE)
...
library(ape)
phyloTree = read.tree(file.path(dataDir, "CTree.tre"))
...
# species names should be row names of TrData rather than a named column
