...
# setwd("U:/all stuff/TEACHING/HMSC/2022 Jyväskylä/case study for exercises")
...
XFormula.reduced = ~ tree+volume+decay
...
m.reduced  = Hmsc(Y=Y, XData = XData,  XFormula = XFormula.reduced,
         TrData = TrData, TrFormula = TrFormula,
         phyloTree = taxonomicTree,
         distr="probit",
         studyDesign = studyDesign, ranLevels=list(site=rL.site))
...
##################################################################################################
models = list(m,m.reduced,m.S)
names(models) = c("with naturalness index","without naturalness index","species richness model")
